...
def download_repo(repo_url, output_file, lang, keep_comments=False, branch_or_tag="main"):
    ...
    if response.status_code == 200:
        ...
        with open(output_file, "w", encoding="utf-8") as outfile:
            for file_path in zip_file.namelist():
                # Skip directories and files that don't meet the minimum content requirement
                if file_path.endswith("/") or not has_sufficient_content(zip_file.read(file_path).decode("utf-8")):
                    continue
                outfile.write(f"// File: {file_path}\n")
                outfile.write(zip_file.read(file_path).decode("utf-8"))
                outfile.write("\n\n")
    else:
        print(f"Failed to download the repository. Status code: {response.status_code}")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    output_file = f"{args.repo_url.split('/')[-1]}.txt"

    download_repo(args.repo_url, output_file, args.branch_or_tag)
